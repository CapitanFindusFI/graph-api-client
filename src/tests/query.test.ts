...
describe("it should correctly generate GraphQL request bodies", () => {
  ...
  it("should work without fields", () => {
    const r = new queryRequest("coordinatorPagination", [], [{
      name: "json",
      type: "JSON"
    }], {});

    const { query } = graphClient.collectRequestBody(GraphQLOperationType.QUERY, r);
    expect(query).toBe("query($json:JSON){coordinatorPagination(json:$json)}");
  });
});
